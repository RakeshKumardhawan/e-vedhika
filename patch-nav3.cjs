...
const nextSiblingStart = code.indexOf('{/* Clock */}');
let navEnd = -1;
if (nextSiblingStart !== -1) {
    navEnd = nextSiblingStart;
} else {
    // find SystemLiveClock
    navEnd = code.indexOf('<SystemLiveClock');
    if (navEnd !== -1) {
       navEnd = code.lastIndexOf('<div', navEnd);
    }
}

if (navStart !== -1 && navEnd !== -1) {
    const oldNav = code.substring(navStart, navEnd);
    // same new code as before...
...
`;
    ...
    console.log("Failed to find navStart or navEnd.", navStart, navEnd);
}
