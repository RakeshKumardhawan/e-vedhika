...
// Replace the current /api/telemetry endpoint with the new comprehensive one
const oldTelemetryEndpointRegex = /app\.post\('\/api\/telemetry'[\s\S]*?\}\);/m;
...
app.post('/api/telemetry', (req, res) => {
  try {
    ...
    // Also save to firestore as requested earlier
    ...
    db.collection("telemetryLogs").add({
      ...
      ip: newRecord.ipAddress,
      createdAt: admin.firestore.FieldValue.serverTimestamp()
    }).catch(console.error);

    return res.status(200).json({
      success: true,
      message: 'Telemetry report logged successfully',
      record: newRecord
    });
  } catch (err: any) {
...
if (content.includes('app.post(\'/api/telemetry\', async (req, res) => {') || content.includes('app.post("/api/telemetry"')) {
    ...
    // Just inject it before the final app.listen or similar
