...
const targetStr3 = `  // 2. Sync URL from state (handles programmatic tab changes, ensuring URL updates correctly)
  useEffect(() => {
...
const newStr3 = `  // 2. Sync URL from state (handles programmatic tab changes, ensuring URL updates correctly)
  useEffect(() => {
    const path = location.pathname.toLowerCase();
    const isFarmerRegistry = path.endsWith("/farmer_registry") || path.endsWith("/farmer-registry");
    if (isFarmerRegistry) return;
