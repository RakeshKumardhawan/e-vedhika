...
const editProfileStart = code.indexOf('function EditProfileModal');
const endOfApp = code.lastIndexOf('</div>\n  );\n}', editProfileStart);
...
`;
    code = code.substring(0, endOfApp) + modalCode + code.substring(endOfApp + 13);
