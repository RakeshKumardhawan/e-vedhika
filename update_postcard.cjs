...
// We should find the exact button tag.
code = code.replace(
  /            <button\n              onClick=\{async \(e\) => \{\n                e\.stopPropagation\(\);\n                if \(requireLoginAlert\(\)\) return;/g,
  `            <motion.button\n              whileTap={{ scale: 0.85 }}\n              onClick={async (e) => {\n                e.stopPropagation();\n                if (requireLoginAlert()) return;`
);

// We need to make sure the closing tag is also replaced for the Heart button
code = code.replace(
  /                \{post\.likes \|\| 0\}\n              <\/span>\n            <\/button>\n          <\/div>\n          <div className="flex items-center gap-2">/g,
  `                {post.likes || 0}\n              </span>\n            </motion.button>\n          </div>\n          <div className="flex items-center gap-2">`
);

// 2. Replace MessageSquare div
code = code.replace(
...
// 3. PostDetail (if it has similar buttons)
// We will skip PostDetail if it's too complex or we can just run a general replace if we know the structure.
// Actually, PostDetail uses the same Heart icon logic, let's check:
